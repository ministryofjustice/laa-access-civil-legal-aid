...
def search():
    ...
    secondary_category: str | None = request.args.get("secondary_category", default=None)

    if category is not None and not FALACategory.is_valid_category(category):
        return abort(404, f"Invalid category: {category}")

    if secondary_category is not None and not FALACategory.is_valid_category(secondary_category):
